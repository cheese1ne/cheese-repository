package com.cheese.db.spring.injector.collector.dialect;
...
import com.cheese.db.spring.injector.collector.method.*;
 ...
 * @author sobann
 */
public class MysqlDialectCollector extends TableInjectMetaCollector {
    ...
    protected List<InjectMeta> getInjectMetas(String schema) {
        ...
        LoadAction action = Actions.getLoad(DevBaseConstant.DEFAULT_SQL_CONFIG_DATASOURCE, DevBaseConstant.DEFAULT_TABLE_META_CONFIG_CODE);
        action.putParam(DevBaseConstant.TABLE_SCHEMA_KEY, schema);
        List<MysqlTableMeta> tableMetaList = db.doActionGetList(action, MysqlTableMeta.class);
        if (Objects.isNull(tableMetaList) || tableMetaList.size() <= DevBaseConstant.ZERO) {
