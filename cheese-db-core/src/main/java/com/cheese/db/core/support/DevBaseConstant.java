...
/**
 * 一些常量配置
 * FIXME 与sql相关的内容需要从此处移出，根据具体的语义环境获取对应的meta sql
 *
 * @author sobann
 */
public interface DevBaseConstant {
    ...
    String DEFAULT_MYSQL_SYS_CONFIG_SQL = "SELECT id, code AS code, dbKey AS dbKey, actiontype AS sqlCommandTypeCode, content AS content FROM sys_sql_config WHERE actiontype IN (0,1,2,3) AND tbnm IS NULL <if test=\"ew.param.code !=null and ew.param.code != '' \"> AND code = #{ew.param.code}</if>";
